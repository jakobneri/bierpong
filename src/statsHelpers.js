...
const personalHeatmapStmt = db.prepare(`
  SELECT cup_index, COUNT(*) AS hits FROM party_hits WHERE hit_by_user_id = ? GROUP BY cup_index
`);
const globalHeatmapStmt = db.prepare(`
  SELECT cup_index, COUNT(*) AS hits FROM party_hits GROUP BY cup_index
`);
const firstHitsStmt = db.prepare(`
  ...
  WHERE ph.hit_by_user_id = ?
    AND ph.sequence = (
      ...
      WHERE ph2.party_id = ph.party_id AND ph2.hit_by_user_id = ph.hit_by_user_id
    )
`);
